...
def test_should_notify_no_due_date(checker):
    ...
    assert checker._should_notify(task, datetime.now(), _default_options()) is False
...
def test_should_notify_future_outside_lead_time(checker):
    ...
    assert (
        checker._should_notify(task, datetime.now(), _default_options(lead_time=24))
        is False
    )
...
def test_should_notify_within_lead_time(checker):
    ...
    assert (
        checker._should_notify(task, datetime.now(), _default_options(lead_time=48))
        is True
    )
...
def test_should_notify_overdue(checker):
    ...
    assert checker._should_notify(task, datetime.now(), _default_options()) is True
...
@patch("custom_components.recurring_todos.notify.date")
def test_build_message_overdue_1_day(mock_date, checker):
...
@patch("custom_components.recurring_todos.notify.date")
def test_build_message_overdue_3_days(mock_date, checker):
...
@patch("custom_components.recurring_todos.notify.date")
def test_build_message_due_today(mock_date, checker):
...
@patch("custom_components.recurring_todos.notify.date")
def test_build_message_due_tomorrow(mock_date, checker):
    mock_date.today.return_value = date(2026, 4, 10)
...
@patch("custom_components.recurring_todos.notify.date")
def test_build_message_due_in_3_days(mock_date, checker):
    mock_date.today.return_value = date(2026, 4, 10)
